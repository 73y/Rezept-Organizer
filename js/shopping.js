...
(() => {
  ...
  function guessReceiptMeta(text) {
    ...
    // Some PDF extractors (or copy/paste) produce "one long line" or glue multiple receipt rows together.
    // We try to split merged lines using price-token heuristics.
    ...
    for (const l of rawLines) {
      ...
      // If we see many prices in a single "line", it's almost certainly multiple receipt rows glued together.
    ...
    for (const lineRaw of lines) {
      ...
      // Keep it tight to avoid matching product names containing "Bar".
      ...
      else if (/\bmwst\b|\bust\b/.test(low)) kind = "misc";
    ...
    function openReceiptGuidedScanModal(state, persist, receiptId, opts = {}) {
      ...
      function render() {
        ...
        msgEl.textContent = "Barcode scannen…";
          ...
          // Fallback: wenn Modal fehlt, trotzdem zuordnen und weiter (besser als hängen bleiben)
          ...
          window.ingredients.openIngredientModal(state, persist, ing, {
            ...
            requirePrice: true,
            onDone: (info) => {
              // Nur wenn wirklich gespeichert wurde: Zuordnung + PurchaseLog + weiter
              if (info && info.saved) {
                cur.matchedIngredientId = ingredientId || null;
                r.updatedAt = new Date().toISOString();
                upsertPurchaseLogFromReceiptItem(state, r, cur);
                ...
                // Cancel/Close: NICHT weiter springen, gleiches Item bleibt offen
                try { window.ui?.toast?.("Nicht gespeichert – bleibt auf diesem Bon-Artikel."); } catch {}
              }
              render();
              startCamera();
            }
    ...
    function openReceiptFreeScanModal(state, persist, receiptId, opts = {}) {
      ...
      async function editIngredientThenResume(ing, opts = {}) {
        ...
        window.ingredients.openIngredientModal(state, persist, ing, {
          ...
          // Erst bearbeiten (Preis/Haltbarkeit). Nur wenn gespeichert wurde: zuordnen + PurchaseLog.
          await editIngredientThenResume(ing, {
            ...
            onDone: (info) => {
              if (info && info.saved) {
                ...
                try { window.ui?.toast?.("Nicht gespeichert – Zuordnung wurde nicht übernommen."); } catch {}
              }
            }
          });
        }
      ...
      window.ingredients.openIngredientModal(state, persist, ing, {
        ...
        async function createAndAssignNew(scannedCode) {
          ...
          window.ingredients.openIngredientModal(state, persist, null, {
            ...
            onDone: (info) => {
              // Nur wenn gespeichert: neue Zutat zuordnen + PurchaseLog + weiter
              if (info && info.saved && info.ingredient && info.ingredient.id) {
                try {
...
// initial (disabled – caused double camera start)
    // render();
    // startCamera();
  }
